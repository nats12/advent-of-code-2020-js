const input = `6 - 8 s: svsssszslpsp
3 - 4 n: gncn
...
function parsedInput(input) {
    ...
    const letter = splitInput[3].split(':')[0];
    const password = splitInput[4];
...
function parsedRawInput(rawInput) {
...
function findValidPasswords(parsedInput) {

    return parsedInput.filter((p) => {
        const firstDigit = p.password[p.min - 1] === p.letter;
        const secondDigit = p.password[p.max - 1] === p.letter;
